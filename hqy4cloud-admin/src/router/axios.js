...
axios.interceptors.response.use(res => {
  ...
  if (res.data.code && res.data.code === 9300) {
    //refresh token
  }

  if (status === 403 && res.data.code === 2001) {
    ElMessageBox.confirm('当前用户已经被锁定,请联系管理员', '系统提示', {
  ...
  if (res.request.responseURL.includes("/oauth2/token")) {
    if (status === 200 && res.data.code && res.data.code === 3001) {
      ElMessage({
        message: "用户名或者密码错误",
        type: 'error'
      })
